...
public class WalletTest extends TestWithWallet {
    ...
    private void basicSpendingCommon(Wallet wallet, Address toAddress, Address destination, boolean testEncryption) throws Exception {
        ...
        BigInteger vHuge = Utils.toNanoCoins(10, 0);
        Wallet.SendRequest req = Wallet.SendRequest.to(destination, vHuge);
        try {
            wallet.completeTx(req);
            fail();
        } catch (InsufficientMoneyException e) {
            Assert.assertEquals(Utils.toNanoCoins(9, 0), e.missing);
        }

        // Prepare to send.
        BigInteger v2 = Utils.toNanoCoins(0, 50);
        req = Wallet.SendRequest.to(destination, v2);
        req.fee = Utils.toNanoCoins(0, 1);
    ...
    private void basicSanityChecks(Wallet wallet, Transaction t, Address fromAddress, Address destination) throws VerificationException {
        ...
        assertEquals(Utils.toNanoCoins(0, 49), t.getOutputs().get(1).getValue());
    ...
    private void spendUnconfirmedChange(Wallet wallet, Transaction t2, KeyParameter aesKey) throws Exception {
        BigInteger v3 = Utils.toNanoCoins(0, 49);
        assertEquals(v3, wallet.getBalance());
        Wallet.SendRequest req = Wallet.SendRequest.to(new ECKey().toAddress(params), Utils.toNanoCoins(0, 48));
        req.aesKey = aesKey;
        ...
        TestUtils.BlockPair bp = TestUtils.createFakeBlock(blockStore, t2, t3);
    ...
    public void customTransactionSpending() throws Exception {
        ...
        BigInteger v2 = Utils.toNanoCoins(0, 50);
        BigInteger v3 = Utils.toNanoCoins(0, 75);
        BigInteger v4 = Utils.toNanoCoins(1, 25);

        Transaction t2 = new Transaction(params);
        t2.addOutput(v2, a2);
        t2.addOutput(v3, a2);
        t2.addOutput(v4, a2);
        Wallet.SendRequest req = Wallet.SendRequest.forTx(t2);
        req.ensureMinRequiredFee = false;
        wallet.completeTx(req);
    ...
    public void balance() throws Exception {
        ...
        BigInteger v1 = Utils.toNanoCoins(5, 0);
        BigInteger v2 = Utils.toNanoCoins(0, 50);
        BigInteger expected = Utils.toNanoCoins(5, 50);
        ...
        // Now spend one coin.
        BigInteger v3 = Utils.toNanoCoins(1, 0);
        ...
        Assert.assertEquals(Utils.toNanoCoins(4, 50), wallet.getBalance(Wallet.BalanceType.ESTIMATED));
        assertFalse(wallet.getBalance(Wallet.BalanceType.AVAILABLE).equals(
                    wallet.getBalance(Wallet.BalanceType.ESTIMATED)));

        // Now confirm the transaction by including it into a block.
        StoredBlock b3 = TestUtils.createFakeBlock(blockStore, spend).storedBlock;
        wallet.receiveFromBlock(spend, b3, BlockChain.NewBlockType.BEST_CHAIN, 0);

        // Change is confirmed. We started with 5.50 so we should have 4.50 left.
        BigInteger v4 = Utils.toNanoCoins(4, 50);
    ...
    public void blockChainCatchup() throws Exception {
        ...
        Transaction send1 = wallet.createSend(new ECKey().toAddress(params), Utils.toNanoCoins(0, 10));
        ...
        Threading.waitForUserCode();
        Assert.assertEquals(Utils.bitcoinValueToFriendlyString(wallet.getBalance()), "0.90");
        assertEquals(null, txn[0]);
        assertEquals(2, confTxns.size());
        assertEquals(txn[1].getHash(), send1.getHash());
        Assert.assertEquals(Utils.bitcoinValueToFriendlyString(bigints[2]), "1.00");
        Assert.assertEquals(Utils.bitcoinValueToFriendlyString(bigints[3]), "0.90");
        // And we do it again after the catchup.
        Transaction send2 = wallet.createSend(new ECKey().toAddress(params), Utils.toNanoCoins(0, 10));
        // What we'd really like to do is prove the official client would accept it .... no such luck unfortunately.
        wallet.commitTx(send2);
        sendMoneyToWallet(send2, AbstractBlockChain.NewBlockType.BEST_CHAIN);
        Assert.assertEquals(Utils.bitcoinValueToFriendlyString(wallet.getBalance()), "0.80");
        Threading.waitForUserCode();
        TestUtils.BlockPair b4 = TestUtils.createFakeBlock(blockStore);
        confTxns.clear();
    ...
    public void balances() throws Exception {
        ...
        assertEquals(BigInteger.ZERO.subtract(Utils.toNanoCoins(0, 10)), send2.getValue(wallet));
    }
...
//    }
    ...
    public void isConsistent_pools() throws Exception {
        ...
        Transaction tx = TestUtils.createFakeTx(params, Utils.toNanoCoins(1, 0), myAddress);
        Address someOtherGuy = new ECKey().toAddress(params);
        TransactionOutput output = new TransactionOutput(params, tx, Utils.toNanoCoins(0, 5), someOtherGuy);
        tx.addOutput(output);
        wallet.receiveFromBlock(tx, null, BlockChain.NewBlockType.BEST_CHAIN, 0);
    ...
    public void doubleSpendUnspendsOtherInputs() throws Exception {
        ...
        Transaction send1 = wallet.createSend(new ECKey().toAddress(params), Utils.toNanoCoins(2, 90));
        // Create a double spend of just the first one.
        Transaction send2 = wallet.createSend(new ECKey().toAddress(params), Utils.toNanoCoins(1, 0));
        send2 = new Transaction(params, send2.bitcoinSerialize());
        // Broadcast send1, it's now pending.
        wallet.commitTx(send1);
        Assert.assertEquals(BigInteger.ZERO, wallet.getBalance());
        ...
        Assert.assertEquals(Utils.toNanoCoins(2, 0), wallet.getBalance());
        assertTrue(wallet.isConsistent());
    }
    ...
    public void doubleSpendIdenticalTx() throws Exception {
        ...
        Transaction send1 = Preconditions.checkNotNull(wallet.createSend(new ECKey().toAddress(params), value2));
        Transaction send2 = Preconditions.checkNotNull(wallet.createSend(new ECKey().toAddress(params), value2));
        send1 = TestUtils.roundTripTransaction(params, send1);
        wallet.commitTx(send2);
        Assert.assertEquals(BigInteger.ZERO, wallet.getBalance());
        sendMoneyToWallet(send1, AbstractBlockChain.NewBlockType.BEST_CHAIN);
        Assert.assertEquals(BigInteger.ZERO, wallet.getBalance());
    ...
    public void doubleSpendFinneyAttack() throws Exception {
        ...
        Transaction send1 = wallet.createSend(new ECKey().toAddress(params), Utils.toNanoCoins(0, 50));
        // Create a double spend.
        Transaction send2 = wallet.createSend(new ECKey().toAddress(params), Utils.toNanoCoins(0, 50));
        send2 = new Transaction(params, send2.bitcoinSerialize());
        // Broadcast send1.
        wallet.commitTx(send1);
    ...
    public void pending1() throws Exception {
        ...
        final Transaction t1 = TestUtils.createFakeTx(params, nanos, myAddress);
        ...
        flags[0] = false;
        flags[1] = false;
        Transaction irrelevant = TestUtils.createFakeTx(params, nanos, new ECKey().toAddress(params));
    ...
    public void keyCreationTime() throws Exception {
        ...
        Assert.assertEquals(now + 60, wallet.getEarliestKeyCreationTime());
        Utils.rollMockClock(60);
        wallet.addKey(new ECKey());
        Assert.assertEquals(now + 60, wallet.getEarliestKeyCreationTime());
    }
...
//    }
    ...
    public void lastBlockSeen() throws Exception {
        BigInteger v1 = Utils.toNanoCoins(5, 0);
        BigInteger v2 = Utils.toNanoCoins(0, 50);
        BigInteger v3 = Utils.toNanoCoins(0, 25);
        Transaction t1 = TestUtils.createFakeTx(params, v1, myAddress);
        Transaction t2 = TestUtils.createFakeTx(params, v2, myAddress);
        Transaction t3 = TestUtils.createFakeTx(params, v3, myAddress);

        Block genesis = blockStore.getChainHead().getHeader();
        Block b10 = TestUtils.makeSolvedTestBlock(genesis, t1);
        Block b11 = TestUtils.makeSolvedTestBlock(genesis, t2);
        Block b2 = TestUtils.makeSolvedTestBlock(b10, t3);
    ...
    public void pubkeyOnlyScripts() throws Exception {
        ...
        wallet.addKey(key1);
        BigInteger value = Utils.toNanoCoins(5, 0);
        Transaction t1 = TestUtils.createFakeTx(params, value, key1);
        if (wallet.isPendingTransactionRelevant(t1))
            wallet.receivePending(t1, null);
        // TX should have been seen as relevant.
        Assert.assertEquals(value, wallet.getBalance(Wallet.BalanceType.ESTIMATED));
        Assert.assertEquals(BigInteger.ZERO, wallet.getBalance(Wallet.BalanceType.AVAILABLE));
        Block b1 = TestUtils.createFakeBlock(blockStore, t1).block;
        chain.add(b1);
    ...
    public void watchingScripts() throws Exception {
        ...
        BigInteger value = Utils.toNanoCoins(5, 0);
        Transaction t1 = TestUtils.createFakeTx(params, value, watchedAddress);
    ...
    public void watchingScriptsConfirmed() throws Exception {
        ...
        Address watchedAddress = key.toAddress(params);
        wallet.addWatchedAddress(watchedAddress);
        Transaction t1 = TestUtils.createFakeTx(params, Utils.CENT, watchedAddress);
        StoredBlock b3 = TestUtils.createFakeBlock(blockStore, t1).storedBlock;
        wallet.receiveFromBlock(t1, b3, BlockChain.NewBlockType.BEST_CHAIN, 0);
        Assert.assertEquals(BigInteger.ZERO, wallet.getBalance());
        Assert.assertEquals(Utils.CENT, wallet.getWatchedBalance());

        // We can't spend watched balances
        Address notMyAddr = new ECKey().toAddress(params);
        wallet.createSend(notMyAddr, Utils.CENT);
    }

    @Test
    public void watchingScriptsSentFrom() throws Exception {
        Assert.assertEquals(2, wallet.getBloomFilterElementCount());

        ECKey key = new ECKey();
        ECKey notMyAddr = new ECKey();
        Address watchedAddress = key.toAddress(params);
        wallet.addWatchedAddress(watchedAddress);
        Assert.assertEquals(3, wallet.getBloomFilterElementCount());

        Transaction t1 = TestUtils.createFakeTx(params, Utils.CENT, watchedAddress);
        Transaction t2 = TestUtils.createFakeTx(params, Utils.COIN, notMyAddr);
        StoredBlock b1 = TestUtils.createFakeBlock(blockStore, t1).storedBlock;
        Transaction st2 = new Transaction(params);
        st2.addOutput(Utils.CENT, notMyAddr);
        st2.addOutput(Utils.COIN, notMyAddr);
        st2.addInput(t1.getOutput(0));
        st2.addInput(t2.getOutput(0));
        wallet.receiveFromBlock(t1, b1, BlockChain.NewBlockType.BEST_CHAIN, 0);
        Assert.assertEquals(4, wallet.getBloomFilterElementCount());
        wallet.receiveFromBlock(st2, b1, BlockChain.NewBlockType.BEST_CHAIN, 0);
        Assert.assertEquals(4, wallet.getBloomFilterElementCount());
        Assert.assertEquals(Utils.CENT, st2.getValueSentFromMe(wallet));
    }
    ...
    public void autosaveImmediate() throws Exception {
        ...
        Transaction t1 = TestUtils.createFakeTx(params, Utils.toNanoCoins(5, 0), key);
        if (wallet.isPendingTransactionRelevant(t1))
            wallet.receivePending(t1, null);
    ...
    public void autosaveDelayed() throws Exception {
        ...
        Block b0 = TestUtils.createFakeBlock(blockStore).block;
        chain.add(b0);
        Sha256Hash hash3 = Sha256Hash.hashFileContents(f);
    ...
    public void spendOutputFromPendingTransaction() throws Exception {
        ...
        t3.addInput(o2);
        t3.signInputs(Transaction.SigHash.ALL, wallet);
    ...
    public void replayWhilstPending() throws Exception {
        ...
        Transaction tx1 = TestUtils.createFakeTx(params, value, myAddress);
        Transaction tx2 = new Transaction(params);
        ...
        TestUtils.BlockPair bp = TestUtils.createFakeBlock(blockStore, tx1);
    ...
    public void feeSolverAndCoinSelectionTest() throws Exception {
        ...
        StoredBlock block = new StoredBlock(TestUtils.makeSolvedTestBlock(blockStore, notMyAddr), BigInteger.ONE, 1);
        Transaction tx1 = TestUtils.createFakeTx(params, BigInteger.ONE, myAddress);
        wallet.receiveFromBlock(tx1, block, AbstractBlockChain.NewBlockType.BEST_CHAIN, 0);
        Transaction tx2 = TestUtils.createFakeTx(params, BigInteger.ONE, myAddress);
        assertTrue(!tx1.getHash().equals(tx2.getHash()));
        wallet.receiveFromBlock(tx2, block, AbstractBlockChain.NewBlockType.BEST_CHAIN, 1);
        Transaction tx3 = TestUtils.createFakeTx(params, BigInteger.TEN, myAddress);
        ...
        Wallet.SendRequest req = Wallet.SendRequest.to(notMyAddr, BigInteger.TEN.add(BigInteger.ONE.add(BigInteger.ONE)));
        req.ensureMinRequiredFee = false;
        assertNotNull(wallet.sendCoinsOffline(req));
        Assert.assertEquals(BigInteger.ZERO, wallet.getBalance());

        // Add some reasonable-sized outputs
        block = new StoredBlock(TestUtils.makeSolvedTestBlock(blockStore, notMyAddr), BigInteger.ONE, 1);
        Transaction tx4 = TestUtils.createFakeTx(params, Utils.COIN, myAddress);
        ...
        Wallet.SendRequest request3 = Wallet.SendRequest.to(notMyAddr, Utils.CENT.subtract(BigInteger.ONE));
        ...
        // ...unless we need it
        Wallet.SendRequest request4 = Wallet.SendRequest.to(notMyAddr, Utils.CENT.subtract(BigInteger.ONE));
        ...
        Wallet.SendRequest request5 = Wallet.SendRequest.to(notMyAddr, Utils.COIN.subtract(Utils.CENT.subtract(BigInteger.ONE)));
        ...
        Wallet.SendRequest request6 = Wallet.SendRequest.to(notMyAddr, Utils.COIN.subtract(Utils.CENT));
        ...
        Wallet.SendRequest request7 = Wallet.SendRequest.to(notMyAddr, Utils.COIN.subtract(Utils.CENT.subtract(BigInteger.valueOf(2)).multiply(BigInteger.valueOf(2))));
        ...
        Wallet.SendRequest request8 = Wallet.SendRequest.to(notMyAddr, Utils.COIN.subtract(Transaction.REFERENCE_DEFAULT_MIN_TX_FEE));
        ...
        Wallet.SendRequest request9 = Wallet.SendRequest.to(notMyAddr, Utils.COIN.subtract(
        ...
        Wallet.SendRequest request10 = Wallet.SendRequest.to(notMyAddr, Utils.COIN.subtract(
        ...
        Wallet.SendRequest request11 = Wallet.SendRequest.to(notMyAddr, Utils.COIN.subtract(
        ...
        Transaction tx5 = TestUtils.createFakeTx(params, Utils.CENT, myAddress);
        wallet.receiveFromBlock(tx5, block, AbstractBlockChain.NewBlockType.BEST_CHAIN, 0);
        Assert.assertEquals(Utils.CENT, wallet.getBalance());
        ...
        block = new StoredBlock(TestUtils.makeSolvedTestBlock(blockStore, notMyAddr), BigInteger.ONE, 1);
        Transaction tx6 = TestUtils.createFakeTx(params, Utils.COIN, myAddress);
        ...
        // Now test feePerKb
        Wallet.SendRequest request15 = Wallet.SendRequest.to(notMyAddr, Utils.CENT);
        for (int i = 0; i < 29; i++)
        ...
        Wallet.SendRequest request16 = Wallet.SendRequest.to(notMyAddr, Utils.CENT);
        ...
        Wallet.SendRequest request17 = Wallet.SendRequest.to(notMyAddr, Utils.CENT);
        for (int i = 0; i < 22; i++)
        ...
        Wallet.SendRequest request18 = Wallet.SendRequest.to(notMyAddr, Utils.CENT);
        for (int i = 0; i < 22; i++)
        ...
        Assert.assertEquals(wallet.getBalance(), Utils.CENT.add(Utils.COIN));
        Wallet.SendRequest request19 = Wallet.SendRequest.to(notMyAddr, Utils.CENT);
        ...
        request19 = Wallet.SendRequest.forTx(request19.tx);
        ...
        Wallet.SendRequest request20 = Wallet.SendRequest.to(notMyAddr, Utils.CENT);
        ...
        request20 = Wallet.SendRequest.forTx(request20.tx);
        ...
        Wallet.SendRequest request21 = Wallet.SendRequest.to(notMyAddr, Utils.CENT);
        ...
        // Same as request 19
        Wallet.SendRequest request25 = Wallet.SendRequest.to(notMyAddr, Utils.CENT);
        ...
        request25 = Wallet.SendRequest.forTx(request25.tx);
        request25.feePerKb = Utils.CENT.divide(BigInteger.valueOf(3));
        request25.ensureMinRequiredFee = false;
        wallet.completeTx(request25);
        Assert.assertEquals(Utils.CENT.subtract(BigInteger.ONE), request25.fee);
    ...
    public void basicCategoryStepTest() throws Exception {
        ...
        while (i <= Utils.CENT.divide(Transaction.REFERENCE_DEFAULT_MIN_TX_FEE).longValue()) {
            Transaction tx = TestUtils.createFakeTxWithChangeAddress(params, Transaction.REFERENCE_DEFAULT_MIN_TX_FEE, myAddress, notMyAddr);
        ...
        Wallet.SendRequest request1 = Wallet.SendRequest.to(notMyAddr, Utils.CENT.add(Transaction.REFERENCE_DEFAULT_MIN_TX_FEE).subtract(BigInteger.ONE));
        ...
        Transaction tx1 = TestUtils.createFakeTxWithChangeAddress(params, Transaction.REFERENCE_DEFAULT_MIN_TX_FEE, myAddress, notMyAddr);
        ...
        Wallet.SendRequest request2 = Wallet.SendRequest.to(notMyAddr, Utils.CENT.add(Transaction.REFERENCE_DEFAULT_MIN_TX_FEE).subtract(BigInteger.ONE));
        ...
        // Give us one more input...
        Transaction tx2 = TestUtils.createFakeTxWithChangeAddress(params, Transaction.REFERENCE_DEFAULT_MIN_TX_FEE, myAddress, notMyAddr);
        ...
        Wallet.SendRequest request3 = Wallet.SendRequest.to(notMyAddr, Utils.CENT.add(Transaction.REFERENCE_DEFAULT_MIN_TX_FEE).subtract(BigInteger.ONE));
        wallet.completeTx(request3);
        assertEquals(BigInteger.ONE, request3.fee);
        assertEquals(request3.tx.getInputs().size(), i - 2); // We should have spent all inputs - 2

        //
        Wallet.SendRequest request4 = Wallet.SendRequest.to(notMyAddr, Utils.CENT.add(Transaction.REFERENCE_DEFAULT_MIN_TX_FEE).subtract(BigInteger.ONE));
        ...
        while (wallet.getBalance().compareTo(Utils.CENT.shiftLeft(1)) < 0) {
            Transaction tx3 = TestUtils.createFakeTxWithChangeAddress(params, Transaction.REFERENCE_DEFAULT_MIN_TX_FEE, myAddress, notMyAddr);
        ...
        Wallet.SendRequest request5 = Wallet.SendRequest.to(notMyAddr, Utils.CENT.add(Transaction.REFERENCE_DEFAULT_MIN_TX_FEE).subtract(BigInteger.ONE));
        wallet.completeTx(request5);
        assertEquals(BigInteger.ONE, request5.fee);
        assertEquals(1, request5.tx.getOutputs().size()); // We should have no change output

        // Give us one more input...
        Transaction tx4 = TestUtils.createFakeTxWithChangeAddress(params, Transaction.REFERENCE_DEFAULT_MIN_TX_FEE, myAddress, notMyAddr);
        ...
        Wallet.SendRequest request6 = Wallet.SendRequest.to(notMyAddr, Utils.CENT.add(Transaction.REFERENCE_DEFAULT_MIN_TX_FEE).subtract(BigInteger.ONE));
        wallet.completeTx(request6);
        assertEquals(BigInteger.ZERO, request6.fee);
    ...
    public void testCategory2WithChange() throws Exception {
        ...
        while (i <= Utils.CENT.divide(Transaction.REFERENCE_DEFAULT_MIN_TX_FEE.multiply(BigInteger.TEN)).longValue()) {
            Transaction tx = TestUtils.createFakeTxWithChangeAddress(params, Transaction.REFERENCE_DEFAULT_MIN_TX_FEE.multiply(BigInteger.TEN), myAddress, notMyAddr);
            tx.getInput(0).setSequenceNumber(i++); // Keep every transaction unique
            wallet.receiveFromBlock(tx, block, AbstractBlockChain.NewBlockType.BEST_CHAIN, i);
        }

        // The selector will choose 2 with MIN_TX_FEE fee
        Wallet.SendRequest request1 = Wallet.SendRequest.to(notMyAddr, Utils.CENT.add(BigInteger.ONE));
        wallet.completeTx(request1);
    ...
    public void feePerKbCategoryJumpTest() throws Exception {
        ...
        // Generate a ton of small outputs
        StoredBlock block = new StoredBlock(TestUtils.makeSolvedTestBlock(blockStore, notMyAddr), BigInteger.ONE, 1);
        Transaction tx = TestUtils.createFakeTx(params, Utils.COIN, myAddress);
        wallet.receiveFromBlock(tx, block, AbstractBlockChain.NewBlockType.BEST_CHAIN, 0);
        Transaction tx2 = TestUtils.createFakeTx(params, Utils.CENT, myAddress);
        wallet.receiveFromBlock(tx2, block, AbstractBlockChain.NewBlockType.BEST_CHAIN, 1);
        Transaction tx3 = TestUtils.createFakeTx(params, BigInteger.ONE, myAddress);
        wallet.receiveFromBlock(tx3, block, AbstractBlockChain.NewBlockType.BEST_CHAIN, 2);

        // Create a transaction who's max size could be up to 1000 (if signatures were maximum size)
        Wallet.SendRequest request1 = Wallet.SendRequest.to(notMyAddr, Utils.COIN.subtract(Utils.CENT.multiply(BigInteger.valueOf(17))));
        for (int i = 0; i < 16; i++)
        ...
        Transaction tx4 = TestUtils.createFakeTx(params, BigInteger.ONE, myAddress);
        wallet.receiveFromBlock(tx4, block, AbstractBlockChain.NewBlockType.BEST_CHAIN, 3);

        // Create a transaction who's max size could be up to 1000 (if signatures were maximum size)
        Wallet.SendRequest request2 = Wallet.SendRequest.to(notMyAddr, Utils.COIN.subtract(Utils.CENT.multiply(BigInteger.valueOf(17))));
        for (int i = 0; i < 16; i++)
    ...
    public void testCompleteTxWithExistingInputs() throws Exception {
        ...
        StoredBlock block = new StoredBlock(TestUtils.makeSolvedTestBlock(blockStore, notMyAddr), BigInteger.ONE, 1);
        Transaction tx1 = TestUtils.createFakeTx(params, Utils.COIN, myAddress);
        wallet.receiveFromBlock(tx1, block, AbstractBlockChain.NewBlockType.BEST_CHAIN, 0);
        Transaction tx2 = TestUtils.createFakeTx(params, Utils.COIN, myAddress); assertTrue(!tx1.getHash().equals(tx2.getHash()));
        wallet.receiveFromBlock(tx2, block, AbstractBlockChain.NewBlockType.BEST_CHAIN, 1);
        Transaction tx3 = TestUtils.createFakeTx(params, Utils.CENT, myAddress);
        wallet.receiveFromBlock(tx3, block, AbstractBlockChain.NewBlockType.BEST_CHAIN, 2);

        Wallet.SendRequest request1 = Wallet.SendRequest.to(notMyAddr, Utils.CENT);
        ...
        Wallet.SendRequest request2 = Wallet.SendRequest.to(notMyAddr, Utils.CENT);
        ...
        Wallet.SendRequest request3 = Wallet.SendRequest.to(notMyAddr, Utils.CENT);
        ...
        Wallet.SendRequest request4 = Wallet.SendRequest.to(notMyAddr, Utils.CENT);
        request4.tx.addInput(tx3.getOutput(0));
        // Now if we manually sign it, completeTx will not replace our signature
        request4.tx.signInputs(Transaction.SigHash.ALL, wallet);
    ...
    public void testEmptyRandomWallet() throws Exception {
        ...
        StoredBlock block = new StoredBlock(TestUtils.makeSolvedTestBlock(blockStore, new ECKey().toAddress(params)), BigInteger.ONE, 1);
        Random rng = new Random();
        for (int i = 0; i < rng.nextInt(100) + 1; i++) {
            Transaction tx = TestUtils.createFakeTx(params, BigInteger.valueOf(rng.nextInt((int) Utils.COIN.longValue())), myAddress);
            wallet.receiveFromBlock(tx, block, AbstractBlockChain.NewBlockType.BEST_CHAIN, i);
        }
        Wallet.SendRequest request = Wallet.SendRequest.emptyWallet(new ECKey().toAddress(params));
    ...
    public void testEmptyWallet() throws Exception {
        ...
        // Add exactly 0.01
        StoredBlock block = new StoredBlock(TestUtils.makeSolvedTestBlock(blockStore, outputKey), BigInteger.ONE, 1);
        Transaction tx = TestUtils.createFakeTx(params, Utils.CENT, myAddress);
        wallet.receiveFromBlock(tx, block, AbstractBlockChain.NewBlockType.BEST_CHAIN, 0);
        Wallet.SendRequest request = Wallet.SendRequest.emptyWallet(outputKey);
        wallet.completeTx(request);
        wallet.commitTx(request.tx);
        Assert.assertEquals(BigInteger.ZERO, wallet.getBalance());
        ...
        block = new StoredBlock(TestUtils.makeSolvedTestBlock(blockStore, outputKey), BigInteger.ONE, 1);
        tx = TestUtils.createFakeTx(params, Utils.CENT, myAddress);
        wallet.receiveFromBlock(tx, block, AbstractBlockChain.NewBlockType.BEST_CHAIN, 0);
        tx = TestUtils.createFakeTx(params, Utils.CENT, myAddress);
        wallet.receivePending(tx, null);
        request = Wallet.SendRequest.emptyWallet(outputKey);
        wallet.completeTx(request);
        wallet.commitTx(request.tx);
        Assert.assertEquals(BigInteger.ZERO, wallet.getBalance());
        assertEquals(Utils.CENT, request.tx.getOutput(0).getValue());
        ...
        tx = TestUtils.createFakeTx(params, Utils.CENT.subtract(BigInteger.ONE), myAddress);
        ...
        tx = TestUtils.createFakeTx(params, outputValue, myAddress);
    ...
    public void keyRotation() throws Exception {
        ...
        wallet.addKey(key1);
        wallet.addKey(key2);
        sendMoneyToWallet(wallet, Utils.CENT, key1.toAddress(params), AbstractBlockChain.NewBlockType.BEST_CHAIN);
        sendMoneyToWallet(wallet, Utils.CENT, key2.toAddress(params), AbstractBlockChain.NewBlockType.BEST_CHAIN);
        sendMoneyToWallet(wallet, Utils.CENT, key2.toAddress(params), AbstractBlockChain.NewBlockType.BEST_CHAIN);
        ...
        final BigInteger THREE_CENTS = Utils.CENT.add(Utils.CENT).add(Utils.CENT);
        ...
        sendMoneyToWallet(wallet, Utils.CENT, key3.toAddress(params), AbstractBlockChain.NewBlockType.BEST_CHAIN);
        assertEquals(0, broadcaster.size());
        ...
        assertEquals(Utils.CENT.subtract(Transaction.REFERENCE_DEFAULT_MIN_TX_FEE), tx.getOutput(0).getValue());
    ...
    public void fragmentedReKeying() throws Exception {
        ...
        Assert.assertEquals(KeyTimeCoinSelector.MAX_SIMULTANEOUS_INPUTS, tx.getInputs().size());
    ...
    public void completeTxPartiallySigned() throws Exception {
        ...
        for (int i = 0; i < req.tx.getInputs().size(); i++) {
            ...
            if (input.getConnectedOutput().getParentTransaction().equals(t1)) {
                Assert.assertArrayEquals(dummySig, input.getScriptSig().getChunks().get(0).data);
