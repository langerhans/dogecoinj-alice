    ...
    public RuleList getBlocksToTest(boolean addSigExpensiveBlocks, boolean runLargeReorgs, File blockStorageFile) throws ScriptException, ProtocolException, IOException {
        ...
        Block b40 = createNextBlock(b39, chainHeadHeight + 13, out12, null);
        {
            ...
            for (int i = 1; i <= numTxes; i++) {
                Transaction tx = new Transaction(params);
                tx.addOutput(new TransactionOutput(params, tx, BigInteger.valueOf(1), new byte[] {ScriptOpCodes.OP_1}));
                tx.addInput(new TransactionInput(params, tx, new byte[]{ScriptOpCodes.OP_1}, lastOutPoint));
            ...
            b41 = createNextBlock(b39, chainHeadHeight + 13, out12, null);
            {
                ...
                for (int i = 1; i <= numTxes; i++) {
                    ...
                    tx.addOutput(new TransactionOutput(params, tx, BigInteger
                            .valueOf(1), new byte[] {ScriptOpCodes.OP_1}));
        ...
        byte[] outScriptBytes = ScriptBuilder.createOutputScript(new ECKey(null, coinbaseOutKeyPubKey)).getProgram();
        {
            ...
            t.addOutput(new TransactionOutput(params, t, BigInteger.valueOf(0), new byte[] {ScriptOpCodes.OP_PUSHDATA1 - 1 }));
            t.addOutput(new TransactionOutput(params, t, BigInteger.valueOf(1), outScriptBytes));
            // Spendable output
            t.addOutput(new TransactionOutput(params, t, BigInteger.ZERO, new byte[] {ScriptOpCodes.OP_1}));
        ...
        Block b62 = createNextBlock(b60, chainHeadHeight + 19, null, null);
        {
            ...
            tx.addOutput(new TransactionOutput(params, tx, BigInteger.ZERO, new byte[] {ScriptOpCodes.OP_TRUE}));
        ...
        Block b65 = createNextBlock(b64, chainHeadHeight + 20, null, null);
        {
            ...
            tx1.addOutput(new TransactionOutput(params, tx1, out19.value, new byte[]{ScriptOpCodes.OP_TRUE}));
            addOnlyInputToTransaction(tx1, out19, 0);
            b65.addTransaction(tx1);
            Transaction tx2 = new Transaction(params);
            tx2.addOutput(new TransactionOutput(params, tx2, BigInteger.ZERO, new byte[]{ScriptOpCodes.OP_TRUE}));
            tx2.addInput(new TransactionInput(params, tx2, new byte[]{ScriptOpCodes.OP_TRUE},
        ...
        {
            Transaction tx1 = new Transaction(params);
            tx1.addOutput(new TransactionOutput(params, tx1, out20.value, new byte[]{ScriptOpCodes.OP_TRUE}));
        ...
        Block b67 = createNextBlock(b65, chainHeadHeight + 21, null, null);
        {
            ...
            tx3.addOutput(new TransactionOutput(params, tx3, out20.value, new byte[]{ScriptOpCodes.OP_TRUE}));
            tx3.addInput(new TransactionInput(params, tx3, new byte[]{ScriptOpCodes.OP_TRUE},
                    new TransactionOutPoint(params, 0, tx1.getHash())));
        ...
        {
            Transaction tx = new Transaction(params);
            tx.addOutput(new TransactionOutput(params, tx, out20.value.subtract(BigInteger.valueOf(9)), new byte[]{ScriptOpCodes.OP_TRUE}));
        ...
        {
            Transaction tx = new Transaction(params);
            tx.addOutput(new TransactionOutput(params, tx, out20.value.subtract(BigInteger.TEN), new byte[]{ScriptOpCodes.OP_TRUE}));
            addOnlyInputToTransaction(tx, out20, 0);
        ...
        Block b70 = createNextBlock(b69, chainHeadHeight + 22, out21, null);
        {
            Transaction tx = new Transaction(params);
            tx.addOutput(new TransactionOutput(params, tx, BigInteger.ZERO, new byte[]{ScriptOpCodes.OP_TRUE}));
            tx.addInput(new TransactionInput(params, tx, new byte[]{ScriptOpCodes.OP_TRUE},
        ...
        {
            b78tx.addOutput(new TransactionOutput(params, b78tx, BigInteger.ZERO, new byte[]{ScriptOpCodes.OP_TRUE}));
        ...
        {
            b79tx.addOutput(new TransactionOutput(params, b79tx, BigInteger.ZERO, new byte[]{ScriptOpCodes.OP_TRUE}));
            b79tx.addInput(new TransactionInput(params, b79tx, new byte[]{ScriptOpCodes.OP_TRUE}, new TransactionOutPoint(params, 0, b78tx.getHash())));
        ...
        if (runLargeReorgs) {
            ...
            for (blockCountAfter1001 = 0; blockCountAfter1001 < TRANSACTION_CREATION_BLOCKS; blockCountAfter1001++) {
                ...
                while (block.getMessageSize() < Block.MAX_BLOCK_SIZE - 500) {
                    Transaction tx = new Transaction(params);
                    tx.addInput(new TransactionInput(params, tx, new byte[] { ScriptOpCodes.OP_TRUE }, lastOutput));
                    tx.addOutput(new TransactionOutput(params, tx, BigInteger.ZERO, new byte[] { ScriptOpCodes.OP_TRUE }));
                    tx.addOutput(new TransactionOutput(params, tx, BigInteger.ZERO, new byte[] { ScriptOpCodes.OP_TRUE }));
